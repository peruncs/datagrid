...
import static peruncs.datagrid.cache.clustered.types.ClusteredCachePropertyParsers.*;
 ...
 * node's updates.</p>
 *
 * <p>The region factory creates one provider per session factory. The provider
 * reads {@link AeronClusteredConfigurationPropertyNames} when the sender and
 * receiver are requested. It provides a synchronous, fail-on-error contract so
 * cache semantics do not depend on transport timing.</p>
 *
 ...
 * it carries no authentication.</p>
 *
 ...
 *
 * <p>The sender waits at most the configured offer timeout (default 5 s) for the

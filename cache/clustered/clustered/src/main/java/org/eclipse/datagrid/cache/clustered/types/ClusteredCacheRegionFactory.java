...
public class ClusteredCacheRegionFactory extends CacheRegionFactory
{
    ...
    )
    {
        ...
        try
        {
            ...
            if (comProviderSetting instanceof Class<?> candidate)
            {
                if (!ClusteredCacheMessageComProvider.class.isAssignableFrom(candidate))
                {
                    throw new CacheException(
                        "Configured clustered communication provider does not implement " +
                            ClusteredCacheMessageComProvider.class.getName() + ": " + candidate.getName());
                }
                comProviderClass = (Class<? extends ClusteredCacheMessageComProvider>)candidate;
            }
            else
            {
                comProviderClass = this.loadClass(comProviderSetting.toString(), settings);
            }

...
///
/// Receivers compare the timestamp with their local value and never move it
/// backwards. The cache and table names identify the local entry to update.
/// [#fromEvent] derives the cache name from
/// [javax.cache.event.CacheEntryEvent#getSource()] and is intended for
/// created and updated events of the timestamp region.
///
...
public record TimestampsRegionUpdateMessage(String cacheName, String tableName, long timestamp) {
    ...
    ///
    /// The timestamp region stores one [Long] timestamp per
    /// [String] table key. The key and value are validated here so every
    ...
    public static <K, V> TimestampsRegionUpdateMessage fromEvent(
        ...
        if (!(key instanceof final String tableName)) {
            throw new IllegalArgumentException("Event key is not of type %s".formatted(String.class.getName()));
        }
        if (!(value instanceof final Long timestamp)) {
            throw new IllegalArgumentException("Event value is not of type %s".formatted(Long.class.getName()));
        }

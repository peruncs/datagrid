...
class AeronReplicationConfigurationTest {
    ...
    void propagatesCustomRetryPolicy() {
        ...
        /// Verifies authentication keys are copied and unsigned operation is explicit.
    @Test
    void validatesAndDefensivelyCopiesAuthenticationKey() {
        final byte[] key = "sixteen-byte-key".getBytes(java.nio.charset.StandardCharsets.US_ASCII);
        final AeronReplicationConfiguration configuration = AeronReplicationConfiguration.builder()
                .authenticationSecret(key)
                .allowUnsignedFrames(false)
                .build();
        key[0] ^= 1;
        assertTrue(configuration.authenticated());
        assertFalse(configuration.allowUnsignedFrames());
        assertArrayEquals("sixteen-byte-key".getBytes(java.nio.charset.StandardCharsets.US_ASCII),
                configuration.authenticationSecret());
        assertThrows(IllegalArgumentException.class, () -> AeronReplicationConfiguration.builder()
                .authenticationSecret(new byte[8]).allowUnsignedFrames(false).build());
        assertThrows(IllegalArgumentException.class, () -> AeronReplicationConfiguration.builder()
                .authenticationSecret("sixteen-byte-key".getBytes(java.nio.charset.StandardCharsets.US_ASCII))
                .build());
    }

        /// Verifies the rotation-overlap key is verified but never required alone.
    @Test
    void validatesTheRotationOverlapKey() {
        final byte[] primary = "primary-secret-key".getBytes(java.nio.charset.StandardCharsets.US_ASCII);
        final byte[] previous = "previous-secret!".getBytes(java.nio.charset.StandardCharsets.US_ASCII);
        final AeronReplicationConfiguration configuration = AeronReplicationConfiguration.builder()
                .authenticationSecret(primary)
                .previousAuthenticationSecret(previous)
                .allowUnsignedFrames(false)
                .build();
        assertArrayEquals(primary, configuration.authenticationSecret());
        assertArrayEquals(previous, configuration.previousAuthenticationSecret());
        assertNull(AeronReplicationConfiguration.defaults().previousAuthenticationSecret());
        assertThrows(IllegalArgumentException.class, () -> AeronReplicationConfiguration.builder()
                .previousAuthenticationSecret(previous)
                .allowUnsignedFrames(false)
                .build());
        assertThrows(IllegalArgumentException.class, () -> AeronReplicationConfiguration.builder()
                .authenticationSecret(primary)
                .previousAuthenticationSecret(primary)
                .allowUnsignedFrames(false)
                .build());
        assertThrows(IllegalArgumentException.class, () -> AeronReplicationConfiguration.builder()
                .authenticationSecret(primary)
                .previousAuthenticationSecret(new byte[8])
                .allowUnsignedFrames(false)
                .build());
        assertThrows(IllegalArgumentException.class, () -> AeronReplicationConfiguration.builder()
                .authenticationSecret(primary)

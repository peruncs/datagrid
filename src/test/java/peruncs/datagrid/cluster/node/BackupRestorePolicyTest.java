...
class BackupRestorePolicyTest {
    ...
    void aeronIdentityFailureIsReportedOnceWithoutACursorCause() {
        final BackupRestorePolicy policy = policy(
                ...
                positionProvider(new NodeLibraryException("no live position")),
    ...
    void unreadableCursorFallsBackToConfiguredIdentity() {
        ...
        final BackupRestorePolicy policy = policy(
                ...
                positionProvider(new NodeLibraryException("no live position")),
                cursorManagerFailure(new NodeLibraryException("cursor is corrupt")));
    ...
    private static BackupRestorePolicy policy(
            ...
            final StoredReplicationCursorManager cursorManager) {
    ...
    private static StoredReplicationCursorManager cursorManager(final ReplicationCursor cursor) {
        return cursorManagerResult(cursor, null);
    }

    private static StoredReplicationCursorManager cursorManagerFailure(final RuntimeException failure) {
        return cursorManagerResult(null, failure);
    }

    private static StoredReplicationCursorManager cursorManagerResult(
            final ReplicationCursor cursor, final RuntimeException failure) {
        return new StoredReplicationCursorManager() {
            @Override

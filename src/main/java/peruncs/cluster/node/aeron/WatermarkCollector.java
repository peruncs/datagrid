    ...
    void ensure() {
        ...
        if (this.settings.topology().role().isWriter()) {
            ...
            this.channel = AeronWatermarkChannel.writer(this.aeron.get(),
                    ...
                    (encoded, offset, length) ->
                    {
                        try {
                            ...
                            if (!this.writerReady.getAsBoolean()) {
                                if (this.settings.archivePolicy().retentionReaders().contains(watermark.readerId()) &&
                                    watermark.clusterId().equals(this.settings.topology().clusterId()) &&
                                    watermark.storeGeneration().equals(this.settings.topology().identity().storeGeneration()) &&
                                    watermark.writerEpoch() == this.settings.topology().epoch()) {
                    ...
                    }, this.settings.timeouts().watermarkCloseTimeoutNanos(),

...
final class StorageFileOperations
{
	...
	static void installStorage(final Path source, final Path destination)
	{
		try
		{
	...
	static void cleanup(final Path path, final Throwable primaryFailure)
	{
		try
		{
	...
	static void ensureNoSymbolicLinks(final Path path) throws IOException
	{
		if (path == null) return;
	...
	static FileAttribute<Set<PosixFilePermission>> ownerOnlyDirectoryAttributes()
	{
	...
	static void forceDirectory(final Path directory) throws IOException
	{

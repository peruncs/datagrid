	...
	implements ClusterReplicationTransportProvider
{
	...
	private static final class Transport implements ClusterReplicationTransport
	{
		...
		private Transport(final String topic, final KafkaPropertiesProvider properties)
		{
		...
		public ReplicationPositionProvider positionProvider(final String streamName)
		{
			...
			{
				@Override public void init() { provider.init(); }
		...
		)
		{
			...
			return new ReplicationHealth()
			{
				...
				@Override public void init() throws NodelibraryException { latest.init(); }
				...
				@Override public boolean isHealthy() { return this.active && storage.isReady(); }

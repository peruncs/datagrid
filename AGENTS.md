...
2. Good OOP design - do the Java entities accurately map to expected APIs and domain concepts? Are Java entities and methods properly named, reflected
   their purpose?
...
22. Memory inefficiencies when packing data in Aeron CBE and Eclipse Serializer. Both formats use memory mapped files/ off-the-heap apis, so we want
...
Do not perform ANY git mutation under any circumstences, do not lose code! Do git mutations only when asked explicitly, then use short, 1-liner commit
mesaages.

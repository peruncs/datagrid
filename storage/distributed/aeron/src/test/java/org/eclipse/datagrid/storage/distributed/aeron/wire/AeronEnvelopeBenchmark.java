...
public final class AeronEnvelopeBenchmark
{
	...
	)
	{
		...
		for (int i = 0; i < warmup; i++) encodeTransaction(target, payload, clusterId, epoch, i,
			payloadLength, chunkSize, chunkCount);
		final AllocationCounter allocation = AllocationCounter.start();
		final long start = System.nanoTime();
		for (int i = 0; i < iterations; i++) encodeTransaction(target, payload, clusterId, epoch, i + warmup,
			payloadLength, chunkSize, chunkCount);
	...
	{
		double nanosecondsPerTransaction() { return elapsedNanoseconds / (double)iterations; }
		long copiedBytesPerTransaction() { return copiedBytes / iterations; }
		long allocatedBytesPerTransaction() { return allocatedBytes < 0 ? -1L : allocatedBytes / iterations; }
	}
	...
	private static final class AllocationCounter
	{
		...
		private static AllocationCounter unavailable() { return new AllocationCounter(null, -1L, -1L); }

		long bytesSinceStart()
		{
			if (this.bean == null) return -1L;
			try { return this.bean.getThreadAllocatedBytes(this.threadId) - this.start; }
			catch (final UnsupportedOperationException | SecurityException ignored) { return -1L; }
		}
	}
}
